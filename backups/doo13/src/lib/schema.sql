...
-- Orders Policies (Users read/insert own orders, Admins manage all)
DROP POLICY IF EXISTS "Public read orders" ON public.orders;
DROP POLICY IF EXISTS "Public manage orders" ON public.orders;

CREATE POLICY "Users read own orders or admin reads all" ON public.orders FOR SELECT USING (user_id = auth.uid() OR public.is_admin());
CREATE POLICY "Users insert own orders" ON public.orders FOR INSERT WITH CHECK (user_id = auth.uid() OR auth.uid() IS NULL);
